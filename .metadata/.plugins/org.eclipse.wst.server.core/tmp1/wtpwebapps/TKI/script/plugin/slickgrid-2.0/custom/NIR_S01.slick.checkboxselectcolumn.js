(function($) {
	...
	function CheckboxSelectColumn(options) {
		...
		function handleSelectedRowsChanged(e, args) {
			...
			if ((0 != selectedRows.length) && (selectedRows.length == _grid.getDataLength())) {
				_grid.updateColumnHeader(_options.columnId,
						"<input type='checkbox' checked='checked'  id='slickGridChkAll'> ", _options.toolTip);
			} else {
				_grid.updateColumnHeader(_options.columnId, "<input type='checkbox'  id='slickGridChkAll'> ",
						_options.toolTip);
			}
		}
		...
		function getColumnDefinition() {
			return {
				...
				name : "<input type='checkbox' id='slickGridChkAll'> ",
		...
		function checkboxSelectionFormatter(row, cell, value, columnDef, dataContext) {
			if (dataContext) {
				if ("Error" === dataContext.syncStatus) {
